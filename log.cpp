 ...
 */
void log_added_request(RequestType requesttype, unsigned int *produced, unsigned int *inBrokerQueue) {
  int idx;
  int total;
  ...
  for (idx=0; idx < RequestTypeN; idx++) {
    if (idx > 0)
      printf(" + ");  /* separator */
    printf("%d %s", inBrokerQueue[idx], producerNames[idx]);
    total += inBrokerQueue[idx];
  }

  printf(" = %d. ", total);

  printf("Added %s.", producers[requesttype]);
  ...
  for (idx=0; idx < RequestTypeN; idx++) {
    ...
    if (idx > 0)
      printf(" + ");  /* separator */
    printf("%d %s", produced[idx], producerNames[idx]);
  }
 ...
 */
void log_removed_request(ConsumerType consumer, RequestType requesttype, unsigned int *consumed, unsigned int *inBrokerQueue) {
  int idx;
  int total;
  ...
  for (idx = 0; idx < RequestTypeN; idx++) {
    ...
    printf("%d %s", consumed[idx], producerNames[idx]);
  }
